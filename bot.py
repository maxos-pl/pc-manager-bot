...
def queue(pc_id, cmd_type, payload=None):
    ...
    r = requests.post(
        ...
        params={"pc_id": pc_id, "cmd_type": cmd_type},
        json=payload or {},
        headers=headers,
        timeout=10,
    )
...
async def start(msg: types.Message):
    await msg.reply(
        "Admin bot. Используй /block <pc_id>, /unblock <pc_id>, "
        "/run <pc_id> <script>, /update <pc_id>, /wake <pc_id>"
    )
...
async def cmd_block(msg: types.Message):
    ...
    payload = (
        {"message": parts[2]}
        if len(parts) > 2
        else {"message": "Этот компьютер заблокирован. Обратитесь к администратору."}
    )
...
if __name__ == "__main__":
    async def on_startup(dp):
        ...
        if webhook and webhook.url:
            print(f"Удаляю активный webhook: {webhook.url}")
